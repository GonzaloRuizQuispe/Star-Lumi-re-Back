...
class admins_db():
    ...
    def conectar_db(self):
        ...
        #Crear Conexion
        self.conexion = sqlite3.connect('BACKEND/database/admins.db')
    ...
    def agregar_user(self,username,password,email,token_header,token_acceso):
        ...
        if not resp:
            #Agregar Valores
            self.db.execute(f"""
                INSERT INTO admins (username,password,email,token_header,token_acceso,balance) VALUES ('{username}','{password}','{email}','{token_header}','{token_acceso}','0')
            """)
    ...
    def find_token_header(self,token_header):
        ...
        self.db.execute(f"""
            SELECT * FROM admins WHERE token_header = '{token_header}'
        """)
    ...
    def find_token_acceso(self,token_acceso):
        ...
        #Se Realiza La Busqueda
        self.db.execute(f"""
            SELECT * FROM admins WHERE token_acceso = '{token_acceso}'
        """)
    ...
    def balance_update(self,id_usuario,balance_update):
        ...
        self.db.execute(f"""
            SELECT * FROM admins WHERE id='{id_usuario}'
        """)
        ...
        self.db.execute(f"""
            UPDATE admins SET balance='{float(resp[0][6])+float(balance_update)}' WHERE id='{int(id_usuario)}'
        """)
